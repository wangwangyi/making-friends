...
class CropController extends Controller{
    ...
    public function postUpload()
    {
        ...
        $image = $manager->make( $photo )->encode('jpg')->save(config('image.path'). $filename_ext );
        if( !$image) {
            return Response::json([
                'status' => 'error',
        ...
        $home = Auth::user();
        $database_image->home_id = $home->id;
        $database_image->save();
        $date = date("Y_m_d");
        return Response::json([
            'status'    => 'success',
            'url'       => env('URL') . '/uploads/images/'.$date . $filename_ext,
            'width'     => $image->width(),
            'height'    => $image->height()
        ], 200);
    }
    ...
    public function postCrop()
    {
        ...
        $image->resize($imgW, $imgH)
            ...
            ->save(config('image.') . 'uploads/big-' . $filename);
        ...
        return Response::json([
            'status' => 'success',
            'url' => env('URL') . 'uploads/big-' . $filename
        ], 200);
